...
- 主动文字使用独立的 `(room_id, sender_id)` 代次、房间活动代次和持久化认领，不复用模型的自由发送能力。默认需 3 次有效互动、成员与房间都静默 45 分钟，并受每成员每天 3 次、每群每天 6 次限制；它只能创建一个文本 Outbox 项，绝不发送媒体。
- 成员的新消息会立即淘汰旧主动代次；停止栅栏、取消、显式关闭主动聊天、发送前当前代次检查和 Chat API 的临界区检查共同阻止过期文字。未挂接的认领、重启中的可恢复任务和已取消终态都会被 SQLite 对账清理。
...
| 存储 | 主要内容 | 默认保留 |
| --- | --- | --- |
| Adapter SQLite | 入站账本、任务提示/输出、工具事件、Artifact、Outbox、用量、项目记忆、成员关系档案、24 小时群时间线、30 天房间摘要和不含正文的房间监听节流状态 | 终态任务与审计 30 天；关系档案默认 90 天 |
...
Adapter 启动时先获取单进程锁，恢复未完成任务并对账 Outbox，恢复结束前 `ready=false`。同一成员的待执行关系摘要会合并后续信号并保留最多四轮内存片段；关系摘要正文只驻留内存，因此启动时会丢弃 `queued/running` 作业。房间摘要仅重放本地 24 小时时间线，因此可将 `running` 作业恢复为 `queued`。`/health` 区分 `live`、`ready` 和 `degraded`；定时清理失败、状态文件过期、Worker 退出或角色卡完整性异常会进入 degraded。`/metrics` 只输出计数、状态和耗时，不包含消息正文。
...
- 角色卡只参与会话表达、Lorebook、示范对话和主动开场节奏，不参与可信身份、任务状态、工具证据、停止栅栏或 Outbox 判定。
