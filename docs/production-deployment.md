...
人格回滚保留 Hermes Adapter 和 Adapter SQLite，只切换回已安装的上一版 Adapter 发布。它会把 `HERMES_WECHAT_SESSION_GENERATION` 设为 `14`；生产仍只使用群级上下文，不会读取旧成员关系数据，也不会删除数据库、重启微信、清理游标或发送状态：
