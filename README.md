...
- 同步与异步路由：普通问答走 Hermes Session；显式搜索及新闻、政策、价格、版本等时效问题自动联网，研究、文件、终端、浏览器和媒体任务走 Run 队列。
...
| 信号 | 行为 |
| --- | --- |
| 默认 | 跟着用户长度接话；普通聊天最多两句、180 字，一句够用就停 |
| 真实任务 | 先执行，状态、证据和产物优先，失败直接说卡点和下一步 |
| `锐评` / `吐槽` / `阴阳一下` / `贴吧老哥模式` | 先分析事实与逻辑，再输出克制的锐评 |
| `正常点` / `认真点` / `退出老哥模式` | 立即恢复默认口吻 |
| 停止、取消、不要图片、只要文字 | 使用标准控制回复，不玩梗 |

人格规则由 [`persona.py`](adapter/app/persona.py) 统一注入并在发送前清理客服开场、主动邀约尾巴和超长草稿，不依赖 Skill，也不参与可信身份、任务状态、工具证据、停止栅栏或 Outbox 判定。
