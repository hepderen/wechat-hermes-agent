...
def test_complete_sunxiaochuan_runtime_bundle_is_pinned_and_checked_before_release_install():
    ...
    assert 'PERSONA_SKILL_PATH.name != "sunxiaochuan.runtime.md"' in SCRIPT
    assert "sunxiaochuan_runtime_integrity" in SCRIPT
    assert "runtime bundle is unexpectedly thin" in SCRIPT
    ...
    assert "sunxiaochuan.runtime.md" in SCRIPT
...
def test_bridge_only_release_requires_a_ready_single_persona_adapter_and_preserves_state():
    ...
    assert "weirdotv@1.0.0+sunxiaochuan@3.0.0" in BRIDGE_RELEASE
...
def test_persona_rollback_rotates_sessions_without_deleting_relationship_data():
    ...
    assert '"HERMES_WECHAT_SESSION_GENERATION": "17"' in PERSONA_ROLLBACK
