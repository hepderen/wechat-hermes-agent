...
def test_production_ports_memory_and_approvals_match_cloud_policy():
    ...
    assert '"HERMES_WECHAT_SESSION_GENERATION": "14"' in SCRIPT
    ...
    assert 'config.pop("mcp_servers", None)' in SCRIPT
    assert '"wechat-production"' not in SCRIPT
...
def test_environment_examples_match_production_generation_and_budget():
    ...
    for relative_path in ("deploy/adapter.env.example",):
        ...
        assert "HERMES_WECHAT_SESSION_GENERATION=14" in example
        ...
        assert "Relationship and proactive-message variables are retired" in example
...
def test_single_sunxiaochuan_section_is_pinned_and_checked_before_release_install():
    assert 'local weirdotv_root="$SOURCE_ROOT/skills/weirdotv-sunxiaochuan"' in SCRIPT
    assert 'local section_path="$SOURCE_ROOT/personas/sunxiaochuan.section.md"' in SCRIPT
    assert 'local section_lock="$SOURCE_ROOT/personas/SOURCE.lock.json"' in SCRIPT
    ...
    assert "b1fa3a4d08206c0210edd527dd2ef30e5ef36bd4eec7401881de873aa75fa922" in SCRIPT
    assert "assert_sunxiaochuan_persona_resources" in SCRIPT
    assert "section SHA-256 mismatch" in SCRIPT
    assert "--exclude 'skills/humanizer-zh-next'" in SCRIPT
    assert "--exclude 'third_party/character-card-spec-v3'" in SCRIPT


def test_single_persona_adapter_release_keeps_the_runtime_pinned_and_reversible():
    assert "EXPECTED_SOURCE_COMMIT" in CCV3_ADAPTER_RELEASE
    assert 'git -C "$SOURCE_ROOT" rev-parse HEAD' in CCV3_ADAPTER_RELEASE
    assert "assert_sunxiaochuan_persona_resources" in CCV3_ADAPTER_RELEASE
    assert "weirdotv@1.0.0+sunxiaochuan@2.0.0" in CCV3_ADAPTER_RELEASE
    assert "skills/humanizer-zh-next" in CCV3_ADAPTER_RELEASE
    assert "skills/sophia" in CCV3_ADAPTER_RELEASE
    assert '"HERMES_WECHAT_SESSION_GENERATION": "14"' in CCV3_ADAPTER_RELEASE
...
def test_bridge_only_release_requires_a_ready_single_persona_adapter_and_preserves_state():
    assert "EXPECTED_SOURCE_COMMIT" in BRIDGE_RELEASE
    assert '"$SOURCE_ROOT/chat-api/db_bridge.py"' in BRIDGE_RELEASE
    assert "weirdotv@1.0.0+sunxiaochuan@2.0.0" in BRIDGE_RELEASE
    assert "Adapter is not Sun Xiaochuan-ready" in BRIDGE_RELEASE
...
def test_persona_rollback_rotates_sessions_without_deleting_relationship_data():
    ...
    assert '"HERMES_WECHAT_SESSION_GENERATION": "15"' in PERSONA_ROLLBACK
